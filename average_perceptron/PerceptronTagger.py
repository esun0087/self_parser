...
def train():
    ...
    training_data = []
    sentence = ([], [])
    for line in codecs.open('data/train.txt', "r", "utf-8"):
        params = [i.strip() for i in line.strip().split() if i.strip()]
        if len(params) != 2:
            break
    ...
    tagger.train(training_data, save_loc=PICKLE, nr_iter=200)
...
def test():
    ...
    sentence = ([], [])
    for line in codecs.open('data/test.txt', 'r', 'utf-8'):
        params = line.split()
        sentence[0].append(params[0])
        if params[0] == '.':
            text = ''
